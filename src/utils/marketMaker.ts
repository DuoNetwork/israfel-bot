...
// 		} else if (bestPriceChange.isBidChange && bestPriceChange.changeAmount < 0 ||
// 			!bestPriceChange.isBidChange && bestPriceChange.changeAmount > 0
// 		) {
// 			await this.createOrderBookSide(pair, orderBookSnapshot.bids[0].price, true, Math.min(3 - orderBookSnapshot.bids.length, 3));
// 			await this.createOrderBookSide(
// 				otherToken.code + '|' + CST.TOKEN_WETH,
// 				this.tokenNavPrices[thisToken.code] + this.tokenNavPrices[otherToken.code] - orderBookSnapshot.bids[0].price, true);
// 		} else if (bestPriceChange.isBidChange && bestPriceChange.changeAmount > 0 ||
// 			!bestPriceChange.isBidChange && bestPriceChange.changeAmount < 0

// 		) {

// 			await this.createOrderBookSide(pair, orderBookSnapshot.asks[0].price, true, Math.min(3 - orderBookSnapshot.asks.length, 3));
// 			await this.createOrderBookSide(
// 				otherToken.code + '|' + CST.TOKEN_WETH,
// 				this.tokenNavPrices[thisToken.code] + this.tokenNavPrices[otherToken.code] - orderBookSnapshot.asks[0].price, false);
// 		}
...
// 	public async createOrderBookSide(pair: string, bestPrice: number, isBid: boolean, level: number = 3) {
// 		if (!this.relayerClient) {
// 			util.logDebug('no relayer client, ignore');
// 			return;
// 		}
// 		const side = isBid ? this.orderBookSnapshots[pair].bids : this.orderBookSnapshots[pair].asks;
// 		for (let i = 0; i++; i < level) {
// 			const levelPrice = util.round(bestPrice + (isBid ? -1 : 1) * i * CST.PRICE_STEP);
// 			const levelAmount = side[i] ? CST.ORDER_BOOK_LEVEL_AMT[i] - side[i].balance : CST.ORDER_BOOK_LEVEL_AMT[i];
// 			if (levelAmount > 0)
// 				await this.relayerClient.addOrder(
// 					this.makerAddress,
// 					pair,
...
// 				bestPriceChange.changeAmount = (orderBookSnapshot.asks[0].price || 0) -
